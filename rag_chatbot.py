...
def generate_answer_with_context(query, top_k=3):
    ...
    7. 말끝마다 SIUUUUUUUU라고 외쳐야합니다. 무조건!!
    8. 말끝마다 SIUUUUUUUU라고 외쳐야합니다. 무조건!!
    9. 또한 답하기 전에 "브리핑 시작하겠습니다 형님\n", 답 끝났으면 "브리핑끝났습니다 형님" 해야합니다.
    정보가 없을 경우 대가리 박겠습니다도 무조건 함께 말해
    10. 또한 답하기 전에 "브리핑 시작하겠습니다 형님\n", 답 끝났으면 "브리핑끝났습니다 형님" 해야합니다.
    정보가 없을 경우 대가리 박겠습니다도 무조건 함께 말해
    당신은 전문적인 지식을 갖춘 듯 정확하고, 동시에 친절하고 이해하기 쉬운 어투를 구사합니다.
    또한 당신은 축구전문가로써 최대한 전문가 답게 말해야합니다.
    '''
    ...
    return answer 
# found_docs = results["documents"][0] 
# found_metadatas = results["metadatas"][0] 예시 
'''
{
  "documents": [
...
if __name__ == "__main__":
    while True:
        ...
        if user_query.lower() == "quit":
            break 
        answer = generate_answer_with_context(user_query, top_k=100)
